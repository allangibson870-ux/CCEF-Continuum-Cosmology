...
*   **Coherence Scale $R_0 \approx \sqrt{A_3 / A_1} \approx 0.8807$**  
    → Intrinsic vacuum coherence length entering packet softening and bath sector.
*   **Derived Sub-Leading Coupling $\gamma_{\text{derived}} = A_3 A_4 \approx 1.466 \times 10^{-1}$**  
...
*   **Interaction Kernel $K(k)$**  
    ...
    with explicit coefficients (for the new $A_i$):  
    $$\Delta = \sqrt{A_1^2 + 4 A_3 A_4} \approx 1.2603$$  
    $$m^2 = \frac{A_1 + \Delta}{2 A_3} \approx 1.46, \quad \Lambda^2 = \frac{-A_1 + \Delta}{2 A_3} \approx 0.17$$  
    $$A = 1/\Delta \approx 0.794, \quad B = -1/\Delta \approx -0.794$$
...
*   **$M_{\text{bath}} \approx \sqrt{A_1 / A_3} \approx 1.14$**  
    → Bath mass gap. Derived as the natural UV cutoff $\Lambda_{\text{UV}}$.
*   **$g_2 \approx 1 / A_3 \approx 1.29$ (natural scaling)**  
    → Gradient-bath coupling. Derived from $A_3 = g_2^2 R_0^6 / M_{\text{bath}}^2$.
*   **$g_1 \approx M_{\text{bath}} / R_0 \approx 1.29$ (for $T_{\text{eff}} \sim 1$)**  
...
*   **$R_0$ (coherence scale)**  
    → Emergent: $R_0 \approx \sqrt{A_3 / A_1} \approx 0.8807$.
...
*   **Energy scale $E_0 \approx 30.608444$ per intrinsic unit**  
    → UV core strain baseline, fixed by the chosen intrinsic–physical matching for the new soliton mass.
*   **Length scale $L_0 \approx 1.610680$ per intrinsic unit**  
    → Characteristic horizon scale in physical units for the new configuration.
*   **The Single Global Phenomenological Remainder Status**: The architecture possesses true system-wide rigidity. The minimal phenomenological remainder is explicitly bound to a single calibration layer ($E_0, L_0$). Once this sub-atomic unit tether is set by hand, all downstream macroscopic solid-state, astrometric, and cosmological metrics are completely un-tunable, parameter-free predictions.
...
*   **Coherence Scale $R_0 \approx 0.8807$**  
    → Natural vacuum coherence radius.  
    **Derived from**: $R_0 = \sqrt{A_3 / A_1}$.
*   **Derived Sub-Leading Coupling $\gamma_{\text{derived}} \approx 1.466 \times 10^{-1}$**  
    → Sub-leading $1/r^3$ correction in the point potential.  
...
*   **Emergent Gyromagnetic Correction Scale**:  
    $$g = 2 \cdot \left[ 1 + \left(\frac{\gamma_{\text{derived}}}{A_1^2}\right) I_{\text{anomaly}} \right] \approx 1.999963$$  
    $$a_{\text{framework}} = \frac{g - 2}{2} \approx -1.84 \times 10^{-5}$$
...
### Global Scale Calibration (Intrinsic → Physical)
*   **Energy Scale $E_0 \approx 30.608444$ per intrinsic unit**  
    → Fixed consistently with $M_{\text{intrinsic}}$ in the new invariant run.
*   **Length Scale $L_0 \approx 1.610680$ per intrinsic unit**  
    → Fixed as the characteristic horizon scale in the new invariant run.
*   **Hydrogen Ground-State Radius (Check):**  
    $R_{\text{peak,phys}} = R_{\text{peak,intrinsic}} \times L_0 \approx 1.992 \times 1.610680$
...
*   **The Parameter-Free Deflection Integral:**  
    ...
    *   **The Absolute Amplitude Verification:** Using the updated $R_0$ and $L_0$, the local interaction ($Z_{\text{bath}} = 1.0$) still yields an un-fitted lensing deflection aligned with the General Relativity photon tensor at the limb.
...
*   **The Evolving Equation of State ($w_{\text{dark}}(z)$):**  
    → Derived parameter-free from the logarithmic derivative of the combined dark layers. The profile transitions smoothly from an early-time dynamic tracking quintessence field down to a stable late-time cosmological constant horizon:
